...
const AuthenticationStore = createContainer(() => {
  ...
  useEffect(() => {
    console.log('[AUTH STORE] Initializing - calling getMyInfo()');
    getMyInfo().then(result => {
      console.log('[AUTH STORE] getMyInfo() succeeded:', result);
      if (typeof result === 'string') {
        throw new Error('Unexpected Response');
      }
      console.log('[AUTH STORE] Setting user data:', result)
      setUserId(result.id);
      setIsStaff(result.isStaff);
      setUsername(result.name);
      setIsAuthenticated(true);
      setIsPending(false);
      console.log('[AUTH STORE] User authenticated:', result.name);
    }).catch(e => {
      console.error('[AUTH STORE] getMyInfo() failed:', e);
      setIsPending(false);
    });
  }, []);
