...
public class AvatarControllerV1 : ControllerBase
{
    ...
    //[HttpGet("avatar/set-avatar-type")]
    [HttpPost("avatar/set-avatar-type")]
    public async Task SetAvatarType()
    {
        long userId = safeUserSession.userId;
        string avatarType = await services.avatar.GetAvatarType(userId);
        FeatureCheck();
        using var cache = ServiceProvider.GetOrCreate<AvatarCache>();
        AvatarService avatarService = Roblox.Services.ServiceProvider.GetOrCreate<AvatarService>();
        await avatarService.UpdateAvatarType(userId, avatarType == "R15" ? 1 : 2);
        await avatarService.RedrawAvatar(userId);
    }

    [HttpGet("avatar/set-avatar-scales")]
    [HttpPost("avatar/set-avatar-scales")]
    public async Task SetWornAssets([Required] string scaletype, [Required] double value)
    {
        long userId = safeUserSession.userId;
        FeatureCheck();
        using var cache = ServiceProvider.GetOrCreate<AvatarCache>();
        AvatarService avatarService = Roblox.Services.ServiceProvider.GetOrCreate<AvatarService>();
        await avatarService.UpdateAvatarScale(userId, value, scaletype);
        await avatarService.RedrawAvatar(userId);
    }
