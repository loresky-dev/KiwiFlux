...
public class SessionMiddleware
{
    ...
    {
        MiddlewareTimer authTimer = new MiddlewareTimer(ctx, "au");
        string currentPath = ctx.Request.Path.ToString().ToLower();
        string FixedPath = Regex.Replace(currentPath, @"\/+", "/");
        var currentRequest = ctx.Request;
        HttpResponse currentResponse = ctx.Response;
        if (currentPath != FixedPath && currentRequest.Headers["User-Agent"].Contains("Roblox/"))
        {
            currentResponse.Redirect($"{currentRequest.Scheme}://{currentRequest.Host}{FixedPath}{currentRequest.QueryString}", permanent: true);
            return;
        }
        try
        {
