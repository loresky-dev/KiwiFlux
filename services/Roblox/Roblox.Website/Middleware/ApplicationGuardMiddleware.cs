...
public class ApplicationGuardMiddleware
{
    ...
    private bool IsAuthorized(HttpContext ctx)
    {
        ...
        if (ctx.Items.ContainsKey(".ROBLOSECURITY") || ctx.Request.Headers.UserAgent.Contains("Roblox"))
            return true;
    ...
    public async Task InvokeAsync(HttpContext ctx)
    {
        ...
        var bypassOk = false;
        var bypassAllowedForPath = allowedPathsForBlockUserAgents.Contains(normalizedPath) || ua.Contains("Roblox/"); // "/" to catch ppl offguard
