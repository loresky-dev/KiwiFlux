...
public class UsersService : ServiceBase, IService
{
    ...
    public async Task<bool> IsUsernameValid(string nameToCheck)
    {
        ...
        if (lowerName.Contains("kike") || lowerName.Contains("goy"))
            return false;
        if (lowerName.Contains("tranny") || lowerName.Contains("K") || lowerName.Contains("fag") || lowerName.Contains("goblina") || lowerName.Contains("dyke") || lowerName.Contains("dick") || lowerName.Contains("cock") || lowerName.Contains("c0ck") || lowerName.Contains("d1ck") || lowerName.Contains("hitler") || lowerName.Contains("hitier"))
            return false;
    ...
    private static string redisKeyPrefix = "sess:v2:";
...
#endif
        // follow by shedletsky
        if (followByShedletsky)
        {
            FriendsService friendsService = new FriendsService();
            await friendsService.FollowerUser(261, result.userId);
        }
        return result;
    }
    ...
    private async Task BeforePurchase(long userIdBuyer, long assetId)
    {
        ...
        const long ownerUserId2 = 4; // todo: get from appsettings
        if (userIdBuyer == ownerUserId || userIdBuyer == ownerUserId2) return;
#if RELEASE
        ...
        if (info.itemRestrictions.Contains("Limited") || info.itemRestrictions.Contains("LimitedUnique"))
        {
            ...
            var owned2 = await GetUserAssets(ownerUserId2, assetId);
            if (!owned.Any() || !owned2.Any())
            {
                try
                {
                    await PurchaseNormalItem(ownerUserId, assetId, info.priceTickets == null ? CurrencyType.Robux : CurrencyType.Tickets);
                    await PurchaseNormalItem(ownerUserId2, assetId, info.priceTickets == null ? CurrencyType.Robux : CurrencyType.Tickets);
                }
                catch (Exception e)
                {
                    // ignore but log
