...
public class AvatarService : ServiceBase, IService
{
    ...
    public async Task<string> GetAvatarType(long userId)
    {
        var result = await db.QuerySingleOrDefaultAsync("SELECT resolved_avatar_type as type FROM user_avatar WHERE user_id = :id", new
        {
            id = userId
        });
        int type = result.type;
        string finalType = type == 2 ? "R15" : "R6";
        return finalType;
    }

    public async Task UpdateAvatarType(long userId, int avatarType)
    {
        await db.ExecuteAsync("UPDATE user_avatar SET resolved_avatar_type = :type WHERE user_id = :id", new
        {
            id = userId,
            type = avatarType
        });
    ...
    public async Task UpdateAvatarScale(long userId, double value, string dbValue)
    {
        await db.ExecuteAsync($"UPDATE user_avatar SET {dbValue} = :value WHERE user_id = :id", new
        {
            id = userId,
            value = value
        });
    ...
    public async Task<ScalesEntryDb> GetAvatarScales(long userId)
    {
        var existingAvatar = await db.QuerySingleOrDefaultAsync(
            "SELECT scale_height, scale_width, scale_head,scale_depth,scale_proportion,scale_body_type FROM user_avatar WHERE user_id = :user_id",
            new
            {
                user_id = userId,
            });
        return new ScalesEntryDb()
        {
            scale_height = existingAvatar.scale_height,
            scale_width = existingAvatar.scale_width,
            scale_head = existingAvatar.scale_head,
            scale_depth = existingAvatar.scale_depth,
            scale_proportion = existingAvatar.scale_proportion,
            scale_body_type = existingAvatar.scale_body_type,
        };
    }
    ...
    public string GetAvatarHash(ColorEntry colors, IEnumerable<long> assetVersionIds, string avatarType = "R6")
    {
        var assets = assetVersionIds.Distinct().ToList();
        assets.Sort((a, b) => a > b ? 1 : a == b ? 0 : -1);
        var str =
            $"avatar-hash-1.7;{avatarType};{string.Join(",", assets)}:{colors.headColorId},{colors.torsoColorId},{colors.leftArmColorId},{colors.rightArmColorId},{colors.leftLegColorId},{colors.rightLegColorId}";
    ...
    public async Task<string> UpdateUserAvatar(long userId, ColorEntry colors, IEnumerable<long> assetIds)
    {
        ...
        return await InTransaction(async (trx) =>
        {
            ...
            return GetAvatarHash(colors, assetVersions, await avatarService.GetAvatarType(userId));
        });
    }
        ...
        AvatarType? avatarType = null, bool forceRedraw = false, bool ignoreLock = false)
    {
        ...
        var request = new Roblox.Rendering.AvatarData()
        {
            ...
            avatarType = await avatarService.GetAvatarType(userId),
