...
public class GameServerService : ServiceBase
{
    ...
    */
    ...
    public async Task<GameServerGetOrCreateResponse> GetServerForPlaceV2(long placeId)
    {
    ...
    public async Task<string> StartGameServer(long placeId, int RCCPort, int networkServerPort, string jobId, int JobExpiration)
    {
        ...
        rccServer.StartInfo.FileName = $"{Configuration.RCCServiceDirectory}RCCService.exe";
        ...
        rccServer.Start();
        string script = $@"loadfile(""https://economy-simulator.org/game/gameserver.ashx"")(""https://economy-simulator.org"", {networkServerPort}, {placeId}, {uni.builderId}, 0, 0)";
        string XML = $@"<?xml version=""1.0"" encoding=""utf-8""?>
            <soap:Envelope xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance""
               xmlns:xsd=""http://www.w3.org/2001/XMLSchema""
               xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"">
                <soap:Body>
                    <OpenJobEx xmlns=""http://economy-simulator.org/"">
                        <job>
                            <id>{jobId}</id>
                            <category>1</category>
                            <cores>2</cores>
    ...
    public static async Task SendSoapRequestToRcc(string URL, string XML, string SOAPAction)
    {
        ...
        {
            RccHttpClient.DefaultRequestHeaders.Add("SOAPAction", $"http://economy-simulator.org/{SOAPAction}");
