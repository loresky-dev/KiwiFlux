...
public class AssetsService : ServiceBase, IService
{
    ...
    public async Task<Dto.Assets.AssetVersionEntry> GetAssetVersionId(long assetVersionId)
    {
        var result = await db.QuerySingleOrDefaultAsync<Dto.Assets.AssetVersionEntry>(
            "SELECT id as assetVersionId, version_number as versionNumber, content_url as contentUrl, content_id as contentId, created_at as createdAt, updated_at as updatedAt, creator_id as creatorId FROM asset_version WHERE id = :id ORDER BY id DESC LIMIT 1",
            new
            {
                id = assetVersionId,
            });
    ...
    public async Task<int> GetAssetType(long assetId)
    {
        var result = await db.QuerySingleOrDefaultAsync("SELECT asset_type as type FROM asset WHERE id = :id", new
        {
            id = assetId
        });
        return result.type;
    ...
    public async Task<long> GetAssetIdByAssetVersionId(long assetversionid)
    {
        var result = await db.QuerySingleOrDefaultAsync("SELECT asset_id as assetversionid FROM asset_version WHERE id = :versionid", new
        {
            versionid = assetversionid
        });
        return result.assetversionid;
    }
    ...
    private async Task CreateAssetTextureThumbnail(long assetId, Models.Assets.Type assetType, CancellationToken? cancellationToken = null, bool isFace = false)
    {
        var latestVersion = await GetLatestAssetVersion(assetId);
        Stream response = isFace ? await Rendering.CommandHandler.RequestFaceThumbnail(assetId, (int) assetType, cancellationToken) : await Rendering.CommandHandler.RequestTextureThumbnail(assetId, (int) assetType, cancellationToken);
    ...
    private async Task CreateImageTextureThumbnail(long assetId, Models.Assets.Type assetType, CancellationToken? cancellationToken = null, bool isFace = false)
    {
        var latestVersion = await GetLatestAssetVersion(assetId);
        string assetRequest = latestVersion.contentUrl;
        var assetContent = await GetAssetContent(assetRequest);
        var key = await UploadAssetContent(assetContent, Configuration.ThumbnailsDirectory, "png");
        await InsertOrReplaceThumbnail(assetId, latestVersion.assetVersionId, key, ModerationStatus.ReviewApproved);
        Thread.Sleep(1000);
    }
    ...
    private async Task CreatePackageThumbnail(long assetId, CancellationToken? cancellationToken = null)
    {
        ...
        */
        List<long> packageAssets = (await GetPackageAssets(assetId)).ToList();
        Stream response = await Rendering.CommandHandler.RequestPackageThumbnail(packageAssets, cancellationToken);
        string key = await UploadAssetContent(response, Configuration.ThumbnailsDirectory, "png");
    ...
    {
        var latestVersion = await GetLatestAssetVersion(assetId);
        var response = await Rendering.CommandHandler.RequestClothingThumbnail(assetId, assetType == Type.Shirt ? "shirt" : "pants", cancellationToken);
        var key = await UploadAssetContent(response, Configuration.ThumbnailsDirectory, "png");
        await InsertOrReplaceThumbnail(assetId, latestVersion.assetVersionId, key, ModerationStatus.ReviewApproved);
    }
    ...
    private async Task CreateBodyPartThumbnail(long assetId, Models.Assets.Type assetType, CancellationToken? cancellationToken = null)
    {
        ...
        var latestVersion = await GetLatestAssetVersion(assetId);
        var response = await Rendering.CommandHandler.RequestBodyPartThumbnail(assetId, type, cancellationToken);
        var key = await UploadAssetContent(response, Configuration.ThumbnailsDirectory, "png");
        await InsertOrReplaceThumbnail(assetId, latestVersion.assetVersionId, key, ModerationStatus.ReviewApproved);
    }
    ...
    public async Task RenderAssetAsync(long assetId, Models.Assets.Type assetType, CancellationToken? cancellationToken = null)
    {
        ...
        switch (assetType)
        {
            ...
            case Models.Assets.Type.Decal:
                thumbRequests.Add(CreateAssetTextureThumbnail(assetId, assetType, cancellationToken, false));
                break;
            case Models.Assets.Type.Face:
                thumbRequests.Add(CreateAssetTextureThumbnail(assetId, assetType, cancellationToken, true));
                break;
            // clothing
